...
app.get("/historical", async (req, res) => {
  ...
  try {
    const url = `https://www.nseindia.com/api/historical/cm/equity?symbol=${symbol.toUpperCase()}&series=[%22EQ%22]&from=${startDate}&to=${endDate}`;
    ...
    return res.status(500).json({ error: "NSE API blocked the request. Try again later." });
  }
});
